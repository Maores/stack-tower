...
(function () {
  ...
  var state = {
    ...
    revivedPost: null,  /* the row the first death posted, replayed on the
                           second screen so the run never posts twice */
    revivedAuto: '',    /* and the exact SAVED AS text it produced */
  ...
  function buildDom() {
    ...
    muteBtn.innerHTML =
      ...
      /* Muted swaps the waves for an X in the space they occupied. A full
         corner-to-corner slash was centred on the viewBox while the speaker
         sits left of it, so half the line struck through nothing. */
      '<path class="hud-mute-x" d="M16 9l6 6M22 9l-6 6"/></svg>';
  ...
  /* The only height allowed to reach the shared board. A revived run posts what
     it had reached before the comeback; the bonus lap is worth points and a
     personal best, never a board row. Both post paths read this, the auto-post
     and the manual SAVE, so neither can leak a post-revive height. */
  function boardScore() {
    return state.revived ? state.bankedScore : state.score;
  }

  /* The roast fired at the first death. The second screen gets its own set so
     the same line never lands twice inside one run. */
  var REVIVE_LINES = [
  ...
  var reviveLineAt = 0;
  function nextReviveLine() {
    var line = REVIVE_LINES[reviveLineAt % REVIVE_LINES.length];
    reviveLineAt++;
    return line;
  }
  ...
  function tryRevive(ev) {
    ...
    /* Snapshot what the first death's post actually produced, so the second
       screen can replay it verbatim instead of posting again. The text is
       captured rather than rebuilt because it already reflects whether the
       row reached the board or only this device. */
    state.revivedPost = state.postedRow;
    state.revivedAuto = state.submitted && !els.autoRow.hidden
      ? els.autoText.textContent : '';
  ...
  function trySave() {
    ...
    /* boardScore, not state.score: after a revive the on-screen score is the
       bonus lap's height, and only the pre-revive height may reach the board. */
  ...
  function applyOver(detail) {
    ...
    /* A revived run posted its pre-revive height and that is what SCORE shows.
       BEST reads finalScore, so it takes the revived height, which is the
       2026-08-06 call: the two numbers legitimately differ, and the offer's own
       copy ("YOUR 38 STILL COUNTS") is what prepared the player for it. */
    var postScore = boardScore();
    els.overScore.textContent = String(postScore);
    els.overBest.textContent = (runMode === 'hard' ? 'HARD BEST ' : 'BEST ') + best;
    els.newBest.hidden = !isNewBest;
    els.overVictim.hidden = true;
    els.overVictim.textContent = '';

    els.quip.textContent = state.revived ? nextReviveLine() : nextQuip();
    ...
    if (warmIsFresh(runMode)) {
      paintSandwich(warmRowsFor('all', runMode), state.revivedPost);
      var wday = warmRowsFor('day', runMode);
      /* The roast belongs to the death that ended the scoring run. A revived
         run already heard it; the second screen has REVIVE_LINES instead. */
      if (wday && !state.revived) { applyRoastOnce(wday); }
    } else {
    ...
    if (state.revived) {
      /* This run already posted, at the first death, with the pre-revive
         height. Replay what that produced; never post a second time. This is
         the integrity condition the whole feature was approved under, and it
         holds here structurally rather than by a check on the score. */
      if (state.revivedPost) {
        state.submitted = true;
        state.postedRow = state.revivedPost;
        els.entry.hidden = true;
        if (state.revivedAuto) { setAutoRow(state.revivedAuto, true); }
        /* wantRoast false: refreshBoard applies a roast of its own when it is
           true, which would overwrite the revive line a few lines above. The
           roast belongs to the death that ended the scoring run, and it already
           fired there. Guarding applyRoastOnce at the call site above is not
           enough on its own; this is the second path into it. */
        refreshBoard(state.revivedPost, false);
      } else {
        /* Nobody posted yet: an unknown player who revived before saving. The
           entry stays the way to post, and trySave reads boardScore, so it
           still sends the pre-revive height. */
        els.entry.hidden = !(postScore > 0);
        refreshBoard(null, true);
      }
    } else if (postScore > 0 && autoName) {
      /* Known player: the run posts itself; the keyboard stays away. */
      els.entry.hidden = true;
      autoSubmit(autoName, postScore);
    } else {
      els.entry.hidden = !(postScore > 0);
      refreshBoard(null, true);
    }
